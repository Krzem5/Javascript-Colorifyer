function colorify(obj,json){
	...
	json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null|undefined)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?|[\,\s\[\]\{\}\(\)\:\;]+)/g,function (m){
		...
		if (/^"/.test(m)){if (/:$/.test(m)){c="red";sp=true}else{c="green"}}
		else if (/true|false/.test(m)){c="blue"}
		else if (/null|undefined/.test(m)){c="magenta"}
		else if (/[\,\s\[\]\{\}\(\)\:\;]+/.test(m)){c="black"}
		m=m.replace(/\n/,"<br>")
		a.push([c,`<span style="color: ${c}">${sp==true?m.replace(":",""):m}</span>`])
		if (sp==true){a.push(["black","<span style=\"color: black\">:</span>"])}
	})
	...
	for (var k of a){o.innerHTML+=k[1]}
	obj.appendChild(o)
}